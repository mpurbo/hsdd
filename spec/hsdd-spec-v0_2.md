...
**Supersedes:** v0.1, and the ad-hoc `system-spec-brainstorm` +
...
| `hsdd-config` | `openspec-config` | Generate and maintain `config.yaml`; per-phase context switch that injects the current phase plus only its consumed contract interfaces and governing ADR decisions. | `openspec/config.yaml` |
...
### 8.1 Default layout (overridable)
...
docs/
  ...
  verify/
    acme.backend.auth.3.verification.md   # one per leaf phase (see 8.3)
contracts/
  ...
  billing-events.md
adr/
  001-auth-provider.md           # cross-cutting decisions (see 9.4)
...
### 8.3 Verification document convention
...
It is generated during the OpenSpec `apply` step (see 9.2) and kept under `docs/`
...
Human responsibility is central, not a rubber stamp. Three mechanisms keep the
...
## 11. Design Decisions and Non-Goals

### 11.1 What HSDD includes and why
...
### 11.2 Non-goals (deliberate exclusions)
...
| Verification doc location | `docs/verify/{phase-id}.verification.md` (Section 8.3). |
...
3. Extend `hsdd-config` to inject governing ADR decisions alongside consumed
   contract interfaces.
4. Seed the default layout and naming into a `conventions.md` template.
